package com.synergizglobal.ircondcs.authenticationinterceptor;
...
public class AuthenticationInterceptor extends HandlerInterceptorAdapter{
	...
	public boolean preHandle(HttpServletRequest request,HttpServletResponse response, Object handler) {
		try {
			...
			if( !request.getRequestURI().equals("/ircondcs/") 
					&& !request.getRequestURI().equals("/ircondcs/login") 
					&& !request.getRequestURI().equals("/") 
					&& !request.getRequestURI().equals("/login")){
			    User userData = (User) request.getSession().getAttribute("USER");
			    if(userData == null){
			    	if(request.getRequestURI().contains("/ircondcs/")){
			    		response.sendRedirect("/ircondcs/");
			    	}else{
